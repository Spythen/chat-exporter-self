...
async def quick_export(
    ...
    # Handle naming for DM/GC/Guild
    if isinstance(channel, discord.DMChannel):
        channel_name = f"{channel.recipient.name} and {bot.user.name if bot else 'User'}"
    elif isinstance(channel, discord.GroupChannel):
        channel_name = channel.name or "Group Chat"
    else:
        channel_name = getattr(channel, "name", "transcript")
    ...
    transcript_file = discord.File(io.BytesIO(transcript.encode()), filename=f"transcript-{channel_name}.html")
